...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class GrantChargeDrainConditionInfo : PausableConditionalTraitInfo
	{
		...
		public readonly int ChargeTicksPerPoint = 0;

		[FieldLoader.Require]
		[Desc("Time it takes to discharge one charge point.")]
		public readonly int DrainTicksPerPoint = 0;
		...
		[Desc("Whether the player can manually disable the condition once activated before the trait fully discharges.")]
		...
		IResolveOrder, IIssueOrder, ISelectionBar, IOrderVoice, ISync, ITick, IIssueDeployOrder
	{
		...
		protected override void Created(Actor self)
		{
			...
			ticks = deployState == TimedDeployState.Charging ? Info.ChargeTicksPerPoint : Info.DrainTicksPerPoint;
		...
		bool CanDeploy()
		{
			...
			if (deployState == TimedDeployState.Active && !Info.CanCancelCondition)
				return false;
		...
		void ITick.Tick(Actor self)
		{
			...
			if (deployState == TimedDeployState.Charging)
			{
				...
				if (currentChargePoints == Info.MaxChargePoints)
				{
					...
					ticks = Info.DrainTicksPerPoint;
				}
				else
					ticks = Info.ChargeTicksPerPoint;
			}
			else
			{
				currentChargePoints--;
