...
LabInfiltrated = function()
	Utils.Do(humans, function(player)
		if player then
			secureLab = player.AddPrimaryObjective("Secure the laboratory before the soviets can react.")
...
CheckLabSecured = function()
	...
	if player1.HasNoRequiredUnits() and player2 and player2.HasNoRequiredUnits() then
		Utils.Do(humans, function(player)
			player.MarkFailedObjective(secureLab)
		end)
	end
