	...
	public class SpawnActorPower : SupportPower
	{
		...
		public override void Activate(Actor self, Order order, SupportPowerManager manager)
		{
			...
			if (info.Actor != null)
			{
				...
				{
					var sonar = w.CreateActor(info.Actor, new TypeDictionary
					{
						new LocationInit(order.TargetLocation),
						new OwnerInit(self.Owner),
					});

					Sound.Play(info.DeploySound, sonar.CenterPosition);

					if (!string.IsNullOrEmpty(info.EffectSequence) && !string.IsNullOrEmpty(info.EffectPalette))
						w.Add(new SpriteEffect(sonar.CenterPosition, w, info.EffectSequence, info.EffectPalette));
	
					sonar.QueueActivity(new Wait(info.LifeTime));
